const allCardPlayer = document.querySelector(".figures-player");
const allCardComputer = document.querySelector(".figures-computer");
const resultGame = document.querySelector(".result");
const counterPlayer = document.querySelector(".counter-player");
const counterComputer = document.querySelector(".counter-computer");
let numberWonPlayer = 0;
let numberWonComputer = 0;

const choice = {
  player: 0,
  computer: 0,
};

const infoCardPlayer = {
  class: "imgPlayer",
  id: ["imgPlayer1", "imgPlayer2", "imgPlayer3", "imgPlayer4", "imgPlayer5"],
  src: [
    "images/rock-player.png",
    "images/paper-player.png",
    "images/scissors-player.png",
    "images/lizard-player.png",
    "images/spock-player.png",
  ],
  text: ["Rock", "Paper", "Scissors", "Lizard", "Spock"],
};
const infoCardComputer = {
  class: "imgComputer",
  id: [
    "imgComputer1",
    "imgComputer2",
    "imgComputer3",
    "imgComputer4",
    "imgComputer5",
  ],
  src: [
    "images/rock-computer.png",
    "images/paper-computer.png",
    "images/scissors-computer.png",
    "images/lizard-computer.png",
    "images/spock-computer.png",
  ],
  text: ["Rock", "Paper", "Scissors", "Lizard", "Spock"],
};
// Рендер гри ================================================================================================
function renderGame(object, placeRendering) {
  placeRendering.innerHTML = `
     <div><img  class=${object.class}  src=${object.src[0]} alt="" id=${object.id[0]} text=${object.text[0]}></div>
     <div><img class=${object.class} src=${object.src[1]} alt="" id=${object.id[1]} text=${object.text[1]}></div>
     <div><img class=${object.class} src=${object.src[2]} alt="" id=${object.id[2]} text=${object.text[2]}></div>
     <div><img class=${object.class} src=${object.src[3]} alt="" id=${object.id[3]} text=${object.text[3]}></div>
     <div><img class=${object.class} src=${object.src[4]} alt="" id=${object.id[4]} text=${object.text[4]}></div>
 `;
  const arrCardPlayer = document.querySelectorAll(".imgPlayer");
  arrCardPlayer.forEach((item) =>
    item.addEventListener("click", figureSelectionByPlayer)
  );
}
// Рендер гри гравця========================================================================================
renderGame(infoCardPlayer, allCardPlayer);

// Вибір фігури гравцем===================================================================================

function figureSelectionByPlayer(event) {
  const choiceElement = event.currentTarget;

  const nameCardPlayer = choiceElement.getAttribute("text");
  choice.player = nameCardPlayer;

  const nameSpanPlayer = document.querySelector(".choice-player");

  // Зміна кольору фігури гравця
  switch (choiceElement.id) {
    case "imgPlayer1":
      infoCardPlayer.src = [
        "images/rock-choice.png",
        "images/paper-player.png",
        "images/scissors-player.png",
        "images/lizard-player.png",
        "images/spock-player.png",
      ];
      break;
    case "imgPlayer2":
      infoCardPlayer.src = [
        "images/rock-player.png",
        "images/paper-choice.png",
        "images/scissors-player.png",
        "images/lizard-player.png",
        "images/spock-player.png",
      ];
      break;
    case "imgPlayer3":
      infoCardPlayer.src = [
        "images/rock-player.png",
        "images/paper-player.png",
        "images/scissors-choice.png",
        "images/lizard-player.png",
        "images/spock-player.png",
      ];
      break;
    case "imgPlayer4":
      infoCardPlayer.src = [
        "images/rock-player.png",
        "images/paper-player.png",
        "images/scissors-player.png",
        "images/lizard-choice.png",
        "images/spock-player.png",
      ];
      break;
    default:
      infoCardPlayer.src = [
        "images/rock-player.png",
        "images/paper-player.png",
        "images/scissors-player.png",
        "images/lizard-player.png",
        "images/spock-choice.png",
      ];
  }
  // Відмальовка назви фігури гравця========================================================================
  nameSpanPlayer.innerHTML = `${nameCardPlayer}`;
  renderGame(infoCardPlayer, allCardPlayer);
  choiceComputer(infoCardComputer);
}

// Рендер гри компьютера=======================================================================================
renderGame(infoCardComputer, allCardComputer);

// Вибір фігури компьютером==================================================================================
function choiceComputer(element) {
  const arrCardComputer = element.text;

  const choiceCardComputer = arrCardComputer[Math.floor(Math.random() * 5)];
  choice.computer = choiceCardComputer;
  gameStep(choice.player, choice.computer);

  //  Зміна кольору фігури компьютера==============================================================================
  switch (choiceCardComputer) {
    case "Rock":
      infoCardComputer.src = [
        "images/rock-choice.png",
        "images/paper-computer.png",
        "images/scissors-computer.png",
        "images/lizard-computer.png",
        "images/spock-computer.png",
      ];
      break;
    case "Paper":
      infoCardComputer.src = [
        "images/rock-computer.png",
        "images/paper-choice.png",
        "images/scissors-computer.png",
        "images/lizard-computer.png",
        "images/spock-computer.png",
      ];
      break;
    case "Scissors":
      infoCardComputer.src = [
        "images/rock-computer.png",
        "images/paper-computer.png",
        "images/scissors-choice.png",
        "images/lizard-computer.png",
        "images/spock-computer.png",
      ];
      break;
    case "Lizard":
      infoCardComputer.src = [
        "images/rock-computer.png",
        "images/paper-computer.png",
        "images/scissors-computer.png",
        "images/lizard-choice.png",
        "images/spock-computer.png",
      ];
      break;
    default:
      infoCardComputer.src = [
        "images/rock-computer.png",
        "images/paper-computer.png",
        "images/scissors-computer.png",
        "images/lizard-computer.png",
        "images/spock-choice.png",
      ];
  }
  renderGame(infoCardComputer, allCardComputer);

  //  Відмальовка назви фігури компьютера==============================================================================================
  const nameSpanComputer = document.querySelector(".choice-computer");
  nameSpanComputer.innerHTML = `${choiceCardComputer}`;
}

// Порівняння результатів вибору========================================================================================================
function gameStep(resultPlayer, resultComputer) {
  if (resultPlayer == resultComputer) {
    return (resultGame.innerHTML = "It's a tie!");
  }
  if (
    resultPlayer == "Rock" &&
    (resultComputer === "Scissors" || resultComputer === "Lizard")
  ) {
    counterPlayer.innerHTML = `${++numberWonPlayer}`;
    return (resultGame.innerHTML = "You Won!");
  }

  if (
    resultPlayer == "Paper" &&
    (resultComputer == "Rock" || resultComputer == "Spock")
  ) {
    counterPlayer.innerHTML = `${++numberWonPlayer}`;
    return (resultGame.innerHTML = "You Won!");
  }
  if (
    resultPlayer == "Scissors" &&
    (resultComputer == "Paper" || resultComputer == "Lizard")
  ) {
    counterPlayer.innerHTML = `${++numberWonPlayer}`;
    return (resultGame.innerHTML = "You Won!");
  }
  if (
    resultPlayer == "Lizard" &&
    (resultComputer == "Paper" || resultComputer == "Spock")
  ) {
    counterPlayer.innerHTML = `${++numberWonPlayer}`;
    return (resultGame.innerHTML = "You Won!");
  }
  if (
    resultPlayer == "Spock" &&
    (resultComputer == "Scissors" || resultComputer == "Rock")
  ) {
    counterPlayer.innerHTML = `${++numberWonPlayer}`;
    return (resultGame.innerHTML = "You Won!");
  } else {
    counterComputer.innerHTML = `${++numberWonComputer}`;
    return (resultGame.innerHTML = "You Lost!");
  }
}
